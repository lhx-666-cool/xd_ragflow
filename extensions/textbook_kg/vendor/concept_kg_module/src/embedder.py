...
from .utils import ensure_dir, log, tokenize_text, truncate_for_embedding
...
class OpenAICompatibleEmbeddingBackend(EmbeddingBackend):
    ...
    def embed_texts(self, texts: list[str]) -> list[list[float]]:
        ...
        if uncached_pairs:
            ...
            total_batches = math.ceil(total_uncached / self.batch_size)
            log(
                "embedder",
                f"Embedding progress start | cached={len(texts) - total_uncached} uncached={total_uncached} batches={total_batches}",
            )
            for start in range(0, len(uncached_pairs), self.batch_size):
                ...
                done_before = start
                done_after = min(start + len(chunk), total_uncached)
                log(
                    "embedder",
                    f"Embedding batch {batch_no}/{total_batches} | items {done_before + 1}-{done_after}/{total_uncached}",
                )
                ...
                log(
                    "embedder",
                    f"Embedding batch {batch_no}/{total_batches} done | progress={done_after / total_uncached:.1%}",
                )
        else:
            log("embedder", f"Embedding progress start | all {len(texts)} vectors loaded from cache")
    ...
    def _prepare_text(self, text: str) -> str:
        return truncate_for_embedding(text, max_tokens=self.max_input_tokens)
...
def build_embedder(settings: dict[str, Any]) -> EmbeddingBackend:
    ...
    if backend_name == "openai_compatible":
        ...
        return OpenAICompatibleEmbeddingBackend(
            ...
            max_input_tokens=int(embedding_settings.get("max_input_tokens", 480)),
