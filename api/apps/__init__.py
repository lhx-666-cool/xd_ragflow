        ...
        try:
            ...
            if user:
                if not user[0].access_token or not user[0].access_token.strip():
                    ...
                    return None
                try:
                    from api.db.services.user_activity_log_service import UserActivityLogService

                    UserActivityLogService.touch(user[0].id)
                except Exception:
                    logging.exception("UserActivityLogService.touch failed in load_user")
                return user[0]
            else:
                return None
